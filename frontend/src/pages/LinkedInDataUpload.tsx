...
const LinkedInDataUpload = () => {
  ...
  const [filteredComments, setFilteredComments] = useState<LinkedInComment[]>([]);
  ...
  const fetchPosts = async (pageNumber = 0, pageSize = 10, searchTerm = '', contentType = '', useFilters = false) => {
    try {
      ...
      if (currentFolder?.category === 'comments') {
        ...
        const commentsApiUrl = `/api/linkedin-data/folders/${folderId}/contents/?page=${pageNumber + 1}&page_size=${pageSize}${searchParam}${projectParam}${filterParams}${sortParams}`;
      ...
      const data = await response.json();
        ...
        const postsApiUrl = `/api/linkedin-data/posts/?${folderParam}&page=${pageNumber + 1}&page_size=${pageSize}${searchParam}${contentTypeParam}${projectParam}${filterParams}${sortParams}`;
  ...
  const fetchFolderDetails = async () => {
    if (folderId) {
      try {
        ...
        const response = await apiFetch(`/api/linkedin-data/folders/${folderId}/?project=${projectId}`);
        if (response.ok) {
          const data = await response.json();
  ...
  const fetchFolderStats = async () => {
    ...
    try {
      ...
      if (currentFolder?.category === 'comments') {
        ...
        const statsApiUrl = `/api/linkedin-data/comments/stats/?${folderParam}${projectParam}`;
      ...
      const data = await response.json();
        ...
        const statsApiUrl = `/api/linkedin-data/posts/stats/?${folderParam}${projectParam}`;
  ...
  const checkServerStatus = async () => {
    try {
      ...
      const endpoint = projectId ? `/api/linkedin-data/folders/?project=${projectId}` : '/api/linkedin-data/folders/';
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      let uploadEndpoint = '/api/linkedin-data/posts/upload_csv/';
      ...
      if (currentFolder?.category === 'comments') {
        uploadEndpoint = '/api/linkedin-data/comments/upload_csv/';
  ...
  const handleDownloadCSV = async (contentType: 'post' | 'reel' | undefined) => {
    try {
      ...
      const response = await apiFetch(
        `/api/linkedin-data/posts/download_csv/?${folderParam}${contentTypeParam}${projectParam}`,
        {
          method: 'GET',
          headers: {
            'Accept': 'application/json',
      ...
      if (contentDisposition) {
        ...
        filename = contentType ? `linkedin_${contentType}s.csv` : 'linkedin_data.csv';
      }
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
          ...
          <Box>
                <Typography variant="h4" component="h1" fontWeight={600} sx={{ mb: 1 }}>
                  {currentFolder ? currentFolder.name : 'LinkedIn Data Management'}
              </Typography>
                <Typography variant="body1" color="text.secondary">
                  {currentFolder ? currentFolder.description || `${currentFolder.category_display || currentFolder.category} data analysis` : 'Manage and analyze your LinkedIn data'}
              </Typography>
              </Box>
            </Box>
          </Box>
            ...
            <Box sx={{ textAlign: 'center' }}>
              ...
              <Typography variant="body2" color="text.secondary">
                Total                   {currentFolder?.category === 'comments'
                  ? 'Comments'
                  : currentFolder?.category === 'articles'
                  ? 'Articles'
                  : 'Posts'}
              </Typography>
            </Box>
                    ...
                    posts.map((post) => (
                      <TableRow key={post.id} hover sx={{ '&:hover': { backgroundColor: 'grey.50' } }}>
                        ...
                        <TableCell>
                          <Chip 
                            size="small" 
                            color={post.content_type === 'article' ? 'secondary' : 'primary'}
                            variant="outlined"
                            label={post.content_type === 'article' ? 'Article' : 'Post'} 
                          />
                      </TableCell>
                      <TableCell>
                          <Typography variant="body2" sx={{ maxWidth: 300 }} noWrap>
              ...
              <Card sx={{ border: '1px solid', borderColor: 'divider' }}>
                <CardContent>
                  ...
                  {currentFolder && currentFolder.category && (
                    <Alert severity="info" sx={{ mb: 2 }}>
                      ...
                      {currentFolder.category === 'comments' 
                        ...
                        : 'Upload posts/articles CSV files here.'
                      }
                    </Alert>
                  )}
                  ...
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                    Upload CSV files containing LinkedIn data. The system automatically detects content type 
                ...
                <CardContent>
                  ...
                  <Stack spacing={2}>
                    ...
                    <Button
                      variant="outlined"
                      startIcon={<DownloadIcon />}
                      onClick={() => handleDownloadCSV('article')}
                      disabled={currentFolder?.category === 'comments'}
                    >
