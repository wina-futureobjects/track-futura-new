...
const LinkedInDataUpload = () => {
  ...
  // Fetch LinkedIn posts with pagination
  const fetchPosts = async (pageNumber = 0, pageSize = 10, searchTerm = '', contentType = '') => {
    try {
      ...
      const response = await fetch(`/api/linkedin-data/posts/?page=${pageNumber + 1}&page_size=${pageSize}${folderParam}${searchParam}${contentTypeParam}`);
  ...
  const fetchFolderDetails = async () => {
    if (folderId) {
      try {
        const response = await fetch(`/api/linkedin-data/folders/${folderId}/`);
        if (response.ok) {
          const data = await response.json();
  ...
  const fetchFolderStats = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/linkedin-data/posts/?${folderParam}&page_size=1000`);
      ...
      if (data && typeof data === 'object' && 'results' in data) {
        ...
        const uniqueUsers = [...new Set(allPosts.map((post: LinkedInPost) => post.user_posted))].length;
        const totalLikes = allPosts.reduce((acc: number, post: LinkedInPost) => acc + post.likes, 0);
        const avgLikes = allPosts.length > 0 ? Math.round(totalLikes / allPosts.length) : 0;
        const verifiedAccounts = allPosts.filter((post: LinkedInPost) => post.is_verified).length;
  ...
  const handlePostUpload = async () => {
    ...
    try {
      ...
      try {
        const response = await fetch('/api/linkedin-data/posts/upload_csv/', {
          method: 'POST',
          body: formData,
          headers: {
        ...
        if (data.success) {
          setUploadSuccess(`Successfully uploaded LinkedIn posts.`);
  ...
  const handleReelUpload = async () => {
    ...
    try {
      ...
      try {
        ...
        if (data.success) {
          setUploadSuccess(`Successfully uploaded LinkedIn reels.`);
          if (data.posts_created > 0 || data.posts_updated > 0) {
            setUploadSuccess(
              `Successfully processed ${data.posts_created + data.posts_updated} reels. ` +
              `Created: ${data.posts_created}, Updated: ${data.posts_updated}, Skipped: ${data.posts_skipped}.`
            );
          }
        ...
        if (error instanceof Error) {
          ...
          setUploadError('Failed to upload reels. Please check your network connection and try again.');
        }
        console.error('Upload error:', error);
      }
    } finally {
  ...
  const handleDownloadCSV = async (contentType: 'post' | 'reel' | undefined) => {
    try {
      let url = `/api/linkedin-data/posts/download_csv/?`;
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Paper sx={{ p: 3, mb: 4 }}>
          ...
          <Box>
            <Box mb={3}>
              <Typography variant="body1" gutterBottom>
                Upload a CSV file containing LinkedIn post data.
              </Typography>
        ...
        {uploadTabValue === 1 && (
          <Box>
            <Box mb={3}>
              <Typography variant="body1" gutterBottom>
                Upload a CSV file containing LinkedIn article data.
              </Typography>
              <Typography variant="body2" color="text.secondary" gutterBottom>
                File must be in CSV format and include the following columns: Post URL, User Name, Post Caption, etc.
              </Typography>
            ...
            >
              {isUploading ? <CircularProgress size={24} /> : 'Upload Articles'}
            </Button>
            <Button
              variant="outlined"
              color="primary"
      ...
      <Paper sx={{ p: 3, mb: 4 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <FormControl sx={{ minWidth: 120, mt: { xs: 2, sm: 0 } }}>
              ...
              >
                ...
                <MenuItem value="reel">Articles</MenuItem>
              </Select>
            </FormControl>
            {/* Search box */}
            <TextField
        ...
        {isLoading ? (
          ...
          <Alert severity="info">
            No LinkedIn data available. Upload some data to get started.
          </Alert>
        ) : (
          <>
            <TableContainer>
              <Table aria-label="LinkedIn posts table">
                <TableHead>
                  <TableRow>
