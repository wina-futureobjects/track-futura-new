...
const LinkedInDataUpload = () => {
  ...
  const fetchPosts = async (pageNumber = 0, pageSize = 10, searchTerm = '', contentType = '') => {
    try {
      ...
      const response = await apiFetch(`/api/linkedin-data/posts/?page=${pageNumber + 1}&page_size=${pageSize}${folderParam}${searchParam}${contentTypeParam}`);
  ...
  const fetchFolderStats = async () => {
    ...
    try {
      ...
      const response = await apiFetch(`/api/linkedin-data/posts/?${folderParam}&page_size=1000`);
  ...
  const handlePostUpload = async () => {
    ...
    try {
      ...
      try {
        const response = await apiFetch('/api/linkedin-data/posts/upload_csv/', {
          method: 'POST',
          body: formData,
          headers: {
