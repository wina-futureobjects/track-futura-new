...
class PlatformConfigValidator:
    ...
    def validate_platform_config(self, platform_key: str, config: Dict) -> List[str]:
        ...
        # Validate dataset_id format (BrightData format: alphanumeric with underscores)
        if not isinstance(config['dataset_id'], str):
            errors.append("dataset_id must be a string")
        elif not config['dataset_id'].replace('_', '').isalnum():
            errors.append("dataset_id must contain only alphanumeric characters and underscores")
